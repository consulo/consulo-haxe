...
public abstract class HaxeReferenceImpl extends HaxeExpressionImpl implements HaxeReference, PsiPolyVariantReference {
  ...
  public ResolveResult[] multiResolve(boolean incompleteCode) {
    final List<? extends PsiElement> elements = ResolveCache.getInstance(getProject()).resolveWithCaching(this, HaxeResolver.INSTANCE, true,
        incompleteCode);
  ...
  public HaxeClassResolveResult resolveHaxeClass() {
    ...
    if (this instanceof HaxeSuperExpression) {
      ...
      final HaxeClassResolveResult superClassResolveResult = superExpression instanceof HaxeReference ? ((HaxeReference) superExpression)
          .resolveHaxeClass() : HaxeClassResolveResult.create(null);
    ...
    if (this instanceof HaxeArrayLiteral) {
      ...
      HaxeClassResolveResult resolveResult = HaxeClassResolveResult.create(HaxeResolveUtil.findClassByQName(getLiteralClassName(getTokenType()
      ), this));
      ...
      if (resolveResultHaxeClass != null && specialization.get(resolveResultHaxeClass, "T") == null) {
        if (isString) {
          ...
          specialization.put(resolveResultHaxeClass, "T", HaxeClassResolveResult.create(HaxeResolveUtil.findClassByQName(haxeClass
    ...
    if (this instanceof HaxeNewExpression) {
      final HaxeClassResolveResult result = HaxeClassResolveResult.create(HaxeResolveUtil.tryResolveClassByQName(((HaxeNewExpression) this)
          .getType()));
    ...
    if (this instanceof HaxeCallExpression) {
      ...
      if (expression instanceof HaxeReference) {
        final HaxeClassResolveResult result = HaxeResolveUtil.getHaxeClassResolveResult(((HaxeReference) expression).resolve(),
  ...
  private static HaxeClassResolveResult tryGetLeftResolveResult(HaxeExpression expression) {
    ...
    return leftReference != null ? leftReference.resolveHaxeClass() : HaxeClassResolveResult.create(PsiTreeUtil.getParentOfType(expression,
        HaxeClass.class));
  }
  ...
  private void bindToPackage(PsiPackage element) {
    final HaxeImportStatementRegular importStatement = HaxeElementGenerator.createImportStatementFromPath(getProject(),
  ...
  private void bindToFile(PsiElement element) {
    ...
    final String importPath = (destinationPackage.isEmpty() ? "" : destinationPackage + ".") + FileUtil.getNameWithoutExtension(((HaxeFile)
        element).getName());

    if (resolve() == null) {
      if (getParent() instanceof HaxeImportStatementRegular && !destinationPackage.isEmpty()) {
        final HaxeImportStatementRegular importStatement = HaxeElementGenerator.createImportStatementFromPath(getProject(), importPath);
        ...
        // qName
        final String newQName = destinationPackage.equals(HaxeResolveUtil.getPackageName(getContainingFile())) ? FileUtil.getNameWithoutExtension(((HaxeFile) element).getName()) : importPath;
        final HaxeImportStatementRegular importStatement = HaxeElementGenerator.createImportStatementFromPath(getProject(), newQName);
        HaxeReferenceExpression referenceExpression = importStatement != null ? importStatement.getReferenceExpression() : null;
        assert referenceExpression != null;
        replace(referenceExpression);
      } else if (UsefulPsiTreeUtil.findImportByClassName(this, getText()) == null && UsefulPsiTreeUtil.findImportWithInByClassName(this,
          getText()) == null && !destinationPackage.isEmpty()) {
        // need add import
  ...
  public Object[] getVariants() {
    final Set<HaxeComponentName> suggestedVariants = new HashSet<HaxeComponentName>();


    // if not first in chain
    // foo.bar.baz
    final HaxeReference leftReference = HaxeResolveUtil.getLeftReference(this);
    if (leftReference != null && getParent() instanceof HaxeReference && leftReference.getText().equals(leftReference.resolveHaxeClass()
        .getHaxeClass().getName())) {
      ...
      addClassNonStaticMembersVariants(suggestedVariants, leftReference.resolveHaxeClass().getHaxeClass(),
          !(leftReference instanceof HaxeThisExpression));
      addUsingVariants(suggestedVariants, leftReference.resolveHaxeClass().getHaxeClass(), HaxeResolveUtil.findUsingClasses(getContainingFile
          ()));
      addChildClassVariants(suggestedVariants, leftReference.resolveHaxeClass().getHaxeClass());
    } else {
      // if chain
    ...
    if (leftTarget instanceof PsiPackage) {
      ...
      PsiPackage rootPackage = PsiPackageManager.getInstance(getElement().getProject()).findPackage("", HaxeModuleExtension.class);
  ...
  private void addImportStatementWithWildcardTypeClassVariants(Set<HaxeComponentName> suggestedVariants, PsiFile psiFile) {
    ...
    for (PsiElement element : importStatementWithWildcardList) {
      List<HaxeNamedComponent> namedSubComponents = UsefulPsiTreeUtil.getImportStatementWithWildcardTypeNamedSubComponents(
          (HaxeImportStatementWithWildcard) element, psiFile);
  ...
  private static void addClassStaticMembersVariants(Set<HaxeComponentName> suggestedVariants, @Nullable HaxeClass haxeClass,
