...
public class HaxeImportUtil
{
	public static List<HaxeImportStatementRegular> findUnusedImports(PsiFile file)
	{
		...
		for(int i = 0; i < usefulImportStatements.size(); i++)
		{
			...
			{
				if(usefulImportStatements.get(i).getReferenceExpression().getText().equals(filteredUsefulImports.get(j).getReferenceExpression()
						.getText()))
				{
					alreadyAdded = true;
					break;
	...
	public static List<HaxeImportStatementWithInSupport> findUnusedInImports(PsiFile file)
	{
		...
		for(int i = 0; i < usefulImportStatementWithInSupports.size(); i++)
		{
			for(int j = 0; j < filteredUsefulImports.size(); j++)
			{
				if(usefulImportStatementWithInSupports.get(i).getReferenceExpression().getText().equals(filteredUsefulImports.get(j)
						.getReferenceExpression().getText()) && usefulImportStatementWithInSupports.get(i).getIdentifier().getText().equals
		...
		List<HaxeImportStatementWithInSupport> uselessImportStatements = new ArrayList<HaxeImportStatementWithInSupport>
				(allImportStatementWithInSupports);
		uselessImportStatements.removeAll(filteredUsefulImports);
	...
	public static List<HaxeImportStatementWithWildcard> findUnusedInImportsWithWildcards(PsiFile file)
	{
		...
		List<HaxeImportStatementWithWildcard> usefulImportStatementWithInSupports = ContainerUtil.findAll(allImportStatementsWithWildcard,
				new Condition<HaxeImportStatementWithWildcard>()
		{
			@Override
			public boolean value(HaxeImportStatementWithWildcard importStatementWithInSupport)
			{
				...
				boolean wildcardForType = UsefulPsiTreeUtil.isImportStatementWildcardForType(qName);
				...
				{
					List<HaxeClass> classesForImportStatementWithWildcard = UsefulPsiTreeUtil.getClassesForImportStatementWithWildcard
							(importStatementWithInSupport);
		...
		for(int i = 0; i < usefulImportStatementWithInSupports.size(); i++)
		{
			...
			{
				if(usefulImportStatementWithInSupports.get(i).getReferenceExpression().getText().equals(filteredUsefulImports.get(j)
						.getReferenceExpression().getText()))
				{
					alreadyAdded = true;
					break;
				}
		...
		List<HaxeImportStatementWithWildcard> uselessImportStatements = new ArrayList<HaxeImportStatementWithWildcard>
				(allImportStatementsWithWildcard);
		uselessImportStatements.removeAll(filteredUsefulImports);
	...
	public static List<HaxeClass> getClassesUsedFromImportStatementWithWildcard(PsiFile file,
			HaxeImportStatementWithWildcard importStatementWithWildcard)
	{
		List<HaxeClass> classesForImportStatementWithWildcard = UsefulPsiTreeUtil.getClassesForImportStatementWithWildcard
