...
public class HaxeSpacingProcessor
{
	...
	public Spacing getSpacing(Block child1, Block child2)
	{
		...
		if((elementType == CLASS_DECLARATION || elementType == ENUM_DECLARATION || elementType == INTERFACE_DECLARATION) && type2 == PLCURLY)
		{
		...
		if(type1 == PLPAREN || type2 == PRPAREN)
		{
			...
			{
				final boolean newLineNeeded = type1 == PLPAREN ? mySettings.METHOD_PARAMETERS_LPAREN_ON_NEXT_LINE : mySettings
						.METHOD_PARAMETERS_RPAREN_ON_NEXT_LINE;
				return addSingleSpaceIf(mySettings.SPACE_WITHIN_METHOD_PARENTHESES, newLineNeeded);
			}
			else if(elementType == CALL_EXPRESSION)
			{
				final boolean newLineNeeded = type1 == PLPAREN ? mySettings.CALL_PARAMETERS_LPAREN_ON_NEXT_LINE : mySettings
			...
			{
				final boolean newLineNeeded = type1 == PLPAREN ? mySettings.PARENTHESES_EXPRESSION_LPAREN_WRAP : mySettings
		...
		//
		if((type1 == COMPARE_OPERATION && EQUALITY_OPERATORS.contains(typeType1)) || (type2 == COMPARE_OPERATION && EQUALITY_OPERATORS.contains
				(typeType2)))
		{
		...
		//
		if((type1 == COMPARE_OPERATION && RELATIONAL_OPERATORS.contains(typeType1)) || (type2 == COMPARE_OPERATION && RELATIONAL_OPERATORS.contains
				(typeType2)))
		{
		...
		// Spacing around  additive operators ( +, -, etc.)
		//
		if((ADDITIVE_OPERATORS.contains(type1) || ADDITIVE_OPERATORS.contains(type2)) && elementType != PREFIX_EXPRESSION)
		{
		...
		//
		if((UNARY_OPERATORS.contains(type1) || UNARY_OPERATORS.contains(type2)) && elementType == PREFIX_EXPRESSION)
		{
	...
	private Spacing setBraceSpace(boolean needSpaceSetting, @CommonCodeStyleSettings.BraceStyleConstant int braceStyleSetting, TextRange textRange)
	{
		...
		{
			final int lineBreaks = braceStyleSetting == CommonCodeStyleSettings.END_OF_LINE || braceStyleSetting == CommonCodeStyleSettings
					.NEXT_LINE_IF_WRAPPED ? 0 : 1;
